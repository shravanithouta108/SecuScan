  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4 border-b-4 border-black pb-4">
        ...
        <span className={`ml-auto px-3 py-0.5 text-[10px] font-black border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] ${accentClass}`}>
          {findings.length}
        </span>
      </div>
      {findings.length === 0 ? (
...
export default function ReportComparison() {
  ...
  const selectClass =
    'w-full bg-charcoal-dark border-4 border-black px-4 py-3 text-[11px] font-black text-silver-bright uppercase tracking-widest italic shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] focus:outline-none'
  ...
  return (
    <div className="min-h-screen bg-charcoal-dark text-silver p-6 md:p-12 space-y-12">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 pb-12 border-b-4 border-silver-bright/10">
        <div className="space-y-4">
          <div className="bg-blue-500 text-black px-4 py-1 text-xs uppercase tracking-widest inline-block shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] font-black">
            Delta_Engine v1.0
          </div>
      ...
      <section className="bg-charcoal border-4 border-black p-10 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] space-y-8">
        ...
        <button
          ...
          className="bg-blue-500 border-4 border-black px-10 py-4 text-[11px] font-black uppercase tracking-widest text-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-1.5 hover:translate-y-1.5 transition-all flex items-center gap-3"
        >
        ...
        {error && (
          <div className="border-4 border-red-500 bg-red-500/10 p-6 flex items-center gap-4">
            <Icon icon={Cancel01Icon} className="text-red-500 shrink-0" />
        ...
        {result && selectedA && selectedB && (
          ...
          >
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                { label: 'New', val: result.newFindings.length, color: 'bg-red-500' },
                { label: 'Fixed', val: result.fixedFindings.length, color: 'bg-green-500' },
                { label: 'Unchanged', val: result.unchangedFindings.length, color: 'bg-gray-400' },
                { label: 'Severity_Changed', val: result.severityChanges.length, color: 'bg-yellow-400' },
              ].map((m) => (
                <div key={m.label} className={`${m.color} border-4 border-black p-8 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] flex flex-col justify-between h-28`}>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-4 border-b-4 border-black pb-4">
                  <Icon icon={Analytics02Icon} className="text-yellow-400" />
                  <h3 className="text-base font-black uppercase tracking-widest italic text-yellow-400">Severity Changes</h3>
                  <span className="ml-auto px-3 py-0.5 text-[10px] font-black border-2 border-black text-yellow-400">
                ...
                {result.severityChanges.length === 0 ? (
                  ...
                  <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-4">
                    {result.severityChanges.map(({ finding, oldSeverity, newSeverity }) => (
                      <motion.div key={finding.id} variants={itemVariants} className="bg-charcoal border-4 border-black p-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] space-y-3">
                        <span className="text-sm font-black text-silver-bright uppercase tracking-tight italic">{finding.title}</span>
                        <div className="flex items-center gap-3">
                          <SeverityBadge severity={oldSeverity} />
                          <Icon icon={ArrowRight01Icon} size={14} className="text-yellow-400" />
